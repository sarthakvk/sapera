...
class ExtractorSpider(scrapy.Spider):
    ...
    def parse( self, response ):
        filename = '../../data/.list-of-algorithms.json'
        ...
        for indx, algo_type in enumerate(algo_class):
            algo_type_xpath = "//div[@id='readme']/*/ul[{indx}]/li//a".format(indx = indx + 1)
            data[algo_type] = [

                make_algo_obj(algo_tag, algo_type) for algo_tag in response.xpath(algo_type_xpath)
            ]
